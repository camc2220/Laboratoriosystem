...
public class InvoicesController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<PagedResult<InvoiceReadDto>>> Get([FromQuery] Guid? patientId, [FromQuery] bool? paid, [FromQuery] DateTime? from, [FromQuery] DateTime? to, [FromQuery] int page=1, [FromQuery] int pageSize=20, [FromQuery] string? sortBy=null, [FromQuery] string sortDir="asc")
    {
        ...
        var data = await pageQ.Select(i => new InvoiceReadDto(i.Id, i.PatientId, i.Patient.FullName, i.Number, i.Amount, i.IssuedAt, i.Paid)).ToListAsync();
    ...
    public async Task<ActionResult<InvoiceReadDto>> Create([FromBody] InvoiceCreateDto dto)
    {
        ...
        var entity = new Domain.Invoice { PatientId = dto.PatientId, Number = dto.Number, Amount = dto.Amount, IssuedAt = dto.IssuedAt ?? DateTime.UtcNow, Paid = dto.Paid };
        _db.Invoices.Add(entity); await _db.SaveChangesAsync();
    ...
    public async Task<IActionResult> Update(Guid id, [FromBody] InvoiceUpdateDto dto)
    {
        ...
        i.Number = dto.Number; i.Amount = dto.Amount; i.IssuedAt = dto.IssuedAt; i.Paid = dto.Paid;
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        var i = await _db.Invoices.FindAsync(id);
        if (i is null) return NotFound();
        _db.Remove(i); await _db.SaveChangesAsync(); return NoContent();
    }
}
