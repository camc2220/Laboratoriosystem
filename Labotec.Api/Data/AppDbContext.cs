...
public class AppDbContext : IdentityDbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        {
            e.HasKey(x => x.Id);
            e.HasOne(x => x.Patient).WithMany(x => x.Appointments).HasForeignKey(x => x.PatientId).OnDelete(DeleteBehavior.Cascade);
        ...
        {
            e.HasKey(x => x.Id);
            e.HasOne(x => x.Patient).WithMany(x => x.Results).HasForeignKey(x => x.PatientId);
        ...
        {
            e.HasKey(x => x.Id);
            e.HasOne(x => x.Patient).WithMany(x => x.Invoices).HasForeignKey(x => x.PatientId);
