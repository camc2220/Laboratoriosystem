...
public class AppointmentsController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<PagedResult<AppointmentReadDto>>> Get([FromQuery] Guid? patientId, [FromQuery] DateTime? from, [FromQuery] DateTime? to, [FromQuery] string? status, [FromQuery] int page=1, [FromQuery] int pageSize=20, [FromQuery] string? sortBy=null, [FromQuery] string sortDir="asc")
    {
        ...
        var data = await pageQ.Select(a => new AppointmentReadDto(a.Id, a.PatientId, a.Patient.FullName, a.ScheduledAt, a.Type, a.Status, a.Notes)).ToListAsync();
    ...
    public async Task<ActionResult<AppointmentReadDto>> Create([FromBody] AppointmentCreateDto dto)
    {
        ...
        var entity = new Domain.Appointment { PatientId = dto.PatientId, ScheduledAt = dto.ScheduledAt, Type = dto.Type, Notes = dto.Notes, Status = "Scheduled" };
        _db.Appointments.Add(entity); await _db.SaveChangesAsync();
    ...
    public async Task<IActionResult> Update(Guid id, [FromBody] AppointmentUpdateDto dto)
    {
        ...
        if (a is null) return NotFound();
        a.ScheduledAt = dto.ScheduledAt; a.Type = dto.Type; a.Status = dto.Status; a.Notes = dto.Notes;
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        var a = await _db.Appointments.FindAsync(id);
        if (a is null) return NotFound();
        _db.Remove(a); await _db.SaveChangesAsync(); return NoContent();
    }
}
